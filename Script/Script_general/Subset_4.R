...
#Definir el periodo
t_0 <- as.Date("2021-07-31")  
t_T <- as.Date("2021-09-30")  
fechas <- seq(from=t_0, to=t_T, by=1)
fechas <- fechas[-which(fechas == "2021-08-09")]
fechas <- fechas[-which(fechas == "2021-08-10")]
...
for (j in 1:N)
{
  ...
  destfile [[j]] =paste0("C:\\Users\\Clase\\Desktop\\Subset4\\",format[j])
  #MacOS:
...
for (m in 1:length(fechas)){
  if(fechas[m]== zip_files[1] ||
     ...
     fechas[m]== zip_files[3] ||
     fechas[m]== zip_files[4] ||
     fechas[m]== zip_files[5] ||
     fechas[m]== zip_files[6] ||
     fechas[m]== zip_files[7] ||
     fechas[m]== zip_files[8] ||
     fechas[m]== zip_files[9] ||
     fechas[m]== zip_files[10] ||
     fechas[m]== zip_files[11] 
  )
...
dataframe_subset_4 <- dataframe_test

write.csv(dataframe_subset_4, 
          "/Volumes/SERGIOB/Proyecto_S.22.08.22/dataframe_subset_4", 
          row.names = FALSE)
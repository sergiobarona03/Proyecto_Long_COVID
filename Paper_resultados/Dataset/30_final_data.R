...
for (j in 1:nrow(final_df)) {
  ...
  # Calcular t (t_T - t_0)
  final_df$t_UCI_desc[j] = difftime(final_df$t_T[j],final_df$t_0[j], units = "days")
  
  # If Outcome = Alive, then t_T = "2021-11-30"
  if (final_df$outcome[j] == "Alive") {
    final_df$t_T[j] = ymd("2021-11-30")}
...
# Censura fija por derecha (C[r] = 120)

for (k in 1:nrow(final_df)) {
  if (final_df$t_UCI[k]  >= 90) {
    final_df$t_UCI[k] = 90
    final_df$outcome[k] = "Alive"
  }
}

...
mann_whitney = data.frame(Variable = c("age", "t_UCI_desc", "t_so"),
                          Sex_W = NA, Sex_p = NA,
                          CCI_W = NA, CCI_p = NA)
...
# LoS ICU
wilcox_sex_LoS = wilcox.test(as.vector((dataset %>% filter(sex == 0))$t_UCI_desc), 
                              as.vector((dataset %>% filter(sex == 1))$t_UCI_desc))
...
# LoS ICU
wilcox_CCI_LoS = wilcox.test(as.vector((dataset %>% filter(CCI == "CCI"))$t_UCI_desc),
                            as.vector((dataset %>% filter(CCI == "non-CCI"))$t_UCI_desc))
